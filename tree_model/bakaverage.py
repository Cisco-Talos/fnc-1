...
def stack_test():
    ...
    sumw = param['w0'] + param['w1'] + param['w2'] + param['w3'] + param['w4']
    
    pred_agree = (x_meta[:,0]*param['w0'] + x_meta[:,4]*param['w1']) / sumw
    pred_disagree = (x_meta[:,1]*param['w0'] + x_meta[:,5]*param['w2']) / sumw
    pred_discuss = (x_meta[:,2]*param['w0'] + x_meta[:,6]*param['w3']) / sumw
    pred_unrelated = (x_meta[:,3]*param['w0'] + x_meta[:,7]*param['w4']) / sumw
    ...
    df_output.to_csv('averaged_2models_cor2.csv', index=False)
...
def run():
    ...
    param_space = {
            ...
            'w2': hp.quniform('w2', 0.01, 2.0, 0.01),
            'w3': hp.quniform('w3', 0.01, 2.0, 0.01),
            'w4': hp.quniform('w4', 0.01, 2.0, 0.01),
            'max_evals': 600
            }
